...
async function checkDuplicateBooking(phone, date, doctorId) {
  return new Promise((resolve, reject) => {
    const sql    = doctorId && doctorId !== 0
      ? 'SELECT id, time, service, doctor_name FROM appointments WHERE phone = ? AND date = ? AND doctor_id = ?'
      : 'SELECT id, time, service, doctor_name FROM appointments WHERE phone = ? AND date = ?'
...
async function cancelAppointment(data) {
  return new Promise((resolve) => {
    db.run(
      'DELETE FROM appointments WHERE phone=? AND date=? AND time=?',
...
async function rescheduleAppointment(oldId, oldData, newDate, newTime) {
  return new Promise((resolve) => {
    // PostgreSQL doesn't support SQLite serialize the same way
    // Use a direct approach: delete then insert
    db.run('DELETE FROM appointments WHERE id = ?', [oldId], function(err) {
      ...
      db.runReturning(
        ...
        function(err2) {
          if (err2) return resolve('slot_taken')
          resolve(this.lastID)
        }
      )
    })
...
module.exports = {
  bookAppointment, checkDuplicateBooking, cancelAppointment,
  findAppointmentsByPhone, rescheduleAppointment,
  getAllServices, getServiceByName
}
